...
<?php 	
		...
		if (empty($nome)&&empty($datad)) {
			...
			$userquery = mysqli_query ($conn, "SELECT * FROM tbHistorico WHERE datau >= '$datad'") or die ("the  query could not be completed. Try again later!");
		}else if (isset($nome) && empty(	$datad)){
			$userquery = mysqli_query ($conn, "SELECT * FROM tbHistorico WHERE nome = '$nome'") or die ("the  query could not be completed. Try again later!");

     	}else {

     		$userquery = mysqli_query ($conn, "SELECT * FROM tbHistorico WHERE datau >= '$datad' AND  nome = '$nome'") or die ("the  query could not be completed. Try again later!");
     	}
